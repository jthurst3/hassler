...
//
root.start =
root.resume = function (notes_data) {
	if (root.currentTime < -1) root.currentTime = -1;
	startAudio(notes_data);
};
...
/*var startAudio = function (currentTime, notes_data, fromCache) {
	...
	for (var n = 0; n < length && messages < 100; n++) {
		eventQueue.push({
					event: event,
					source: MIDI.noteOn(0, data[n][0]+60, 127, .8*data[n][1][0]),
					interval: scheduleTracking(0, data[n][0]+60, .8*data[n][1][0], .8*(data[n][1][1] - data[n][1][0]), 144, 127)
				});
		queuedTime += data[n][1];
...
var startAudio = function(notes_times) {
