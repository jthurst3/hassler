class PostsController < ApplicationController
	...
	def create
		@post = Post.new(params[:post].permit(:title, :text, :caption, :url, :image_caption, :file))
		if @post.save
			redirect_to @post
		else
			render 'new'
	...
	def update
		...
		if params.has_key?(:post)
			@post.update(params[:post].permit(:title, :text, :caption, :url, :image_caption, :file))
			redirect_to @post
		else
			render 'edit'
		end
	...
	private
	def post_params
		params.require(:post).permit(:title, :text, :caption, :url, :image_caption, :file)
	end
end
